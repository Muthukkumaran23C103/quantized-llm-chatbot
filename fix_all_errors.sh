#!/bin/bash
# COMPLETE FIX - Resolves all errors and warnings
# Fixes Gradio warnings, port conflicts, missing packages, and FastAPI issues

echo "🛠️  FIXING ALL ERRORS - ULTIMATE QUANTIZED LLM CHATBOT"
echo "===================================================="
...
pkill -f "ultimate_chatbot" 2>/dev/null || true
sleep 3
...
echo -e "${BLUE}🔧 Step 3: Installing/fixing dependencies...${NC}"
# Install the missing accelerate package and upgrade others
pip install --upgrade pip setuptools wheel

# Core requirements with fixes
cat > requirements_fixed.txt << 'EOF'
# Fixed Gradio (latest version)
gradio>=4.44.1

# AI/ML with accelerate (fixes model loading error)
torch>=2.0.0
transformers>=4.35.0
accelerate>=0.24.0
sentence-transformers>=2.2.0

# FastAPI with updated syntax
fastapi>=0.104.1
...
echo -e "${BLUE}🔧 Step 4: Creating FIXED backend (resolves FastAPI warnings)...${NC}"

# Create a FIXED backend that resolves the FastAPI and model loading issues
cat > fixed_backend.py << 'EOF'
#!/usr/bin/env python3
"""
...
echo -e "${GREEN}✅ Step 5: All fixes applied!${NC}"

echo ""
echo -e "${GREEN}🎉 EVERYTHING IS FIXED!${NC}"
echo -e "${BLUE}================================${NC}"
echo ""
echo -e "${GREEN}✅ Fixed Issues:${NC}"
echo "   • Gradio deprecation warnings (type='messages')"
echo "   • FastAPI on_event deprecation (lifespan events)"
echo "   • Missing accelerate package"
echo "   • torch_dtype deprecation"
echo "   • Port conflicts (auto cleanup)"
echo "   • Model loading errors"
echo ""
echo -e "${YELLOW}🚀 Ready to Launch:${NC}"
echo ""
echo -e "${GREEN}Option 1 - Launch Both (Recommended):${NC}"
echo "   Terminal 1: python fixed_backend.py"
echo "   Terminal 2: python FIXED_ultimate_chatbot.py"
echo ""
echo -e "${GREEN}Option 2 - Auto Launch:${NC}"
read -p "Start the FIXED system now? (y/n): " -r
if [[ $REPLY =~ ^[Yy]$ ]]; then
    echo ""
    echo -e "${BLUE}🚀 Starting FIXED Backend...${NC}"
    python fixed_backend.py > logs/backend_fixed.log 2>&1 &
    BACKEND_PID=$!
    
    echo -e "${YELLOW}⏳ Waiting for backend to start...${NC}"
    sleep 8
    
    echo -e "${BLUE}🚀 Starting FIXED Frontend...${NC}"
    python FIXED_ultimate_chatbot.py &
    FRONTEND_PID=$!
    
    echo ""
    echo -e "${GREEN}🎉 FIXED SYSTEM RUNNING!${NC}"
    echo -e "${GREEN}=========================${NC}"
    echo ""
    ...
    cleanup() {
        echo ""
        echo -e "${YELLOW}🛑 Stopping FIXED system...${NC}"
    ...
    echo ""
    echo -e "${BLUE}📋 Manual Launch Commands:${NC}"
    echo -e "${GREEN}   python fixed_backend.py${NC}"
